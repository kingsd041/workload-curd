...
function Create() {
	...
	result=$(rancher kubectl apply -f $path-$loop)
...
function Read() {
	...
	if [ "$op" == "$OP_CREATE" ]; then
		result=$( rancher kubectl get pods -l app=nginx-$loop | grep Running |wc -l| grep -o "[^ ]\+\( \+[^ ]\+\)*" ) # Running数量
		...
		result=$( rancher kubectl get pods -l app=nginx-$loop | grep nginx | wc -l | grep -o "[^ ]\+\( \+[^ ]\+\)*" ) # 总数量
		...
		total=$( rancher kubectl get pods -l app=nginx-$loop | grep nginx | wc -l | grep -o "[^ ]\+\( \+[^ ]\+\)*" ) # 总数量
		running=$( rancher kubectl get pods -l app=nginx-$loop | grep Running |wc -l| grep -o "[^ ]\+\( \+[^ ]\+\)*" ) # Running数量
...
function Delete() {
	...
	# loop=$1
	path="$DEFAULT_DEPLOYMENT_FILE_DIR/$DEFAULT_DEPLOYMENT_FILE"
	result=$(rancher kubectl delete -f $path-$loop)
